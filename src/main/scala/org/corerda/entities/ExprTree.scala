...
object ExprTree {
  ...
  def stem[A](ops: Transformer[A])(prev: ExprTree[A]): ExprTree[A] =
    ExprTree(prev.eval.map(ops.f))
  ...
  def root[A](ops: Writer[A])(prev: ExprTree[A]): ExprTree[A] =
    ExprTree(prev.eval.map(ops.write))
}
